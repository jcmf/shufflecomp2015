module.exports =
  lake:
    name: 'By the Lake'
    author: 'Eldridge Murray'
    download: 'http://www.nigeljayne.ca/scd2downloads/By%20the%20lake.ulx'
    play: 'http://iplayif.com/?story=http%3A%2F%2Fwww.nigeljayne.ca/scd2downloads/By%20the%20lake.ulx'
    songs: [
      name: 'The Ghost Who Walks'
      author: 'Karen Elson'
      play: 'https://www.youtube.com/watch?v=D10iE2YVe60'
    ]
  comrade:
    name: 'Comrade'
    author: 'Urist Uristson'
    download: 'http://www.nigeljayne.ca/scd2downloads/Comrade.gblorb'
    play: 'http://iplayif.com/?story=http%3A%2F%2Fwww.nigeljayne.ca/scd2downloads/Comrade.gblorb'
    walkthrough: 'http://www.nigeljayne.ca/scd2downloads/comrade%20walkthru.txt'
    songs: [
      name: 'Rossiya'
      author: 'Laibach'
      play: 'https://www.youtube.com/watch?v=YmnardtFcEU'
    ,
      name: 'Cha Cha on the Moon',
      author: 'Pat Reader',
      play: 'https://www.youtube.com/watch?v=gcYQtj6avVE'
    ,
      name: 'Rocket Man'
      author: 'Iron Horse'
      play: 'https://www.youtube.com/watch?v=forqmom3YuY'
    ,
      name: 'Russian Roulette'
      author: 'Rihanna'
      play: 'https://youtu.be/ZQ2nCGawrSY'
    ]
  everything:
    name: 'Everything We Do Is Games'
    author: 'Doug Orleans'
    download: 'http://www.nigeljayne.ca/scd2downloads/Everything%20We%20Do%20Is%20Games.zblorb'
    play: 'http://steak.place.org/games/everything-we-do-is-games/'
    songs: [
      name: '''4'33"'''
      author: 'John Cage (composer), Lawrence Foster (conductor), and the BBC Symphony Orchestra'
      play: 'https://www.youtube.com/watch?v=zY7UK-6aaNA'
    ]
  east:
    name: 'Heading East'
    author: 'Alex Davies'
    download: 'http://www.nigeljayne.ca/scd2downloads/heading_east.z5'
    play: 'http://iplayif.com/?story=http%3A%2F%2Fwww.nigeljayne.ca/scd2downloads/heading_east.z5'
    hints: 'http://www.nigeljayne.ca/scd2downloads/he-hints.txt'
    walkthrough: 'http://www.nigeljayne.ca/scd2downloads/he-walkthrough.txt'
    songs: [
      name: 'Vacant Night Sky'
      author: 'Mecca Normal'
      play: 'http://www.youtube.com/watch?v=o9k6jffqDsg'
    ]
  molly:
    name: 'Molly and the Butter Thieves'
    author: 'Cosmic Hamster'
    download: 'http://www.nigeljayne.ca/scd2downloads/Molly%20and%20the%20Butter%20Thieves.gblorb'
    play: 'http://www.nigeljayne.ca/scd2downloads/mbtplay.html'
    songs: [
      name: "Teddy Bears' Picnic"
      author: 'Anne Murray'
      play: 'https://www.youtube.com/watch?v=uxFIGWm9M6w'
    ,
      name: 'The Caterpillar'
      author: 'The Cure'
      play: 'https://www.youtube.com/watch?v=76xQgxR2FdQ'
    ,
      name: 'Lullabye for a Stormy Night'
      author: 'Vienna Teng'
      play: 'https://www.youtube.com/watch?v=mlmhMS_luX8'
    ,
      name: 'Highway 5'
      author: 'Marian Call'
      play: 'https://www.youtube.com/watch?v=4qYRF2FEjyE'
    ,
      name: 'Rain and Snow'
      author: 'The Be Good Tanyas'
      play: 'https://www.youtube.com/watch?v=Goeh9WsTyDw'
    ]
  songbird:
    name: 'The Songbird, the Woman and the Silver Casket'
    author: 'Taryn Quinn'
    download: 'http://www.nigeljayne.ca/scd2downloads/songbird.a3c'
    songs: [
      name: 'Real Emotional Girl'
      author: 'Randy Newman'
      play: 'https://www.youtube.com/watch?v=5eSottwYIwo'
    ,
      name: 'Little Birdie'
      author: 'Vince Guaraldi'
      play: 'https://www.youtube.com/watch?v=om9DIDru7pI'
    ,
      name: 'Rosie'
      author: "The Prisoners at the Mississippi State Penitentiary's Parchman Farm work camp; recorded by Alan Lomax"
      play: 'https://www.youtube.com/watch?v=9ch5IWTavUc'
    ]
  starry:
    name: 'Starry Seeksorrow'
    author: 'Ayla Rose'
    download: 'http://www.nigeljayne.ca/scd2downloads/Starry%20Seeksorrow.zblorb'
    play: 'http://iplayif.com/?story=http%3A%2F%2Fwww.nigeljayne.ca/scd2downloads/Starry%20Seeksorrow.zblorb'
    hints: 'http://www.nigeljayne.ca/scd2downloads/seeksorrow%20hints.txt'
    songs: [
      name: 'The Violet Hour'
      author: 'Dolls Come to Life'
      play: 'https://www.youtube.com/watch?v=j61UGd_WDj0'
    ]
  water:
    name: 'When the Land Goes Under the Water'
    author: 'Nikephoros De Kloet'
    download: 'http://www.nigeljayne.ca/scd2downloads/When%20the%20Land%20Goes%20Under%20the%20Wa.gblorb'
    play: 'http://www.nigeljayne.ca/scd2downloads/wtlplay.html'
    songs: [
      name: "What'cha Gonna Do"
      author: 'Abigail Washburn and Bela Fleck'
      play: 'https://www.youtube.com/watch?v=tTh3XqMFyH0'
    ]
  ansible:
    name: 'Ansible'
    author: 'Jacques Frechet'
    play: 'http://toastball.net/games/ansible/'
    songs: [
      name: "Save the World / Don't You Worry Child"
      author: 'Pentatonix'
      play: 'https://www.youtube.com/watch?v=7YAcMgoSpDE'
    ]
  mavis:
    name: 'False Mavis'
    author: 'Litany Brisket'
    play: 'http://www.nigeljayne.ca/scd2downloads/falsemavis.html'
    download: 'http://www.nigeljayne.ca/scd2downloads/falsemavis.zip'
    songs: [
      name: 'Burning Rope'
      author: 'Genesis'
      play: 'https://www.youtube.com/watch?v=SWZzXg_plrk'
    ,
      name: 'Long Lankin'
      author: 'Steeleye Span'
      play: 'https://www.youtube.com/watch?v=sSUH6YPM9oI'
    ]
  mayor:
    name: 'The Mayor and the Machine'
    author: 'J. Marie'
    play: 'http://eristhenia.net/twine/machine.html'
    songs: [
      name: 'This Is Not A Game'
      author: 'Skunk Anansie'
      play: 'https://www.youtube.com/watch?v=iMzCvIBxFJA'
    ]
  pool:
    name: 'Pool Dominator'
    author: 'Destiny Spearmint'
    play: 'http://www.nigeljayne.ca/scd2downloads/pool%20dominator.html'
    songs: [
      name: 'Fuego'
      author: 'Bomba Estereo'
      play: 'http://youtu.be/Td1hajshtGA'
      lyrics: 'http://lyricstranslate.com/en/fuego-fire.html-1'
    ]
  dreamland:
    name: 'Running Down a Dreamland by Wes Eas'
    author: 'Wes Eas'
    play: 'http://www.nigeljayne.ca/scd2downloads/dreamland_shuffle_final.html'
    songs: [
      name: 'The Big Strong Man'
      author: 'Two Man Gentleman Band'
      play: 'https://www.youtube.com/watch?v=XM9N-26ugrU'
    ,
      name: "Runnin' Down a Dream"
      author: 'Tom Petty and the Heartbreakers'
      play: 'https://www.youtube.com/watch?v=Y1D3a5eDJIs'
    ,
      name: 'Super Mario Bros OST'
      author: 'Koji Kondo'
      play: 'https://www.youtube.com/watch?v=uhscMsBhNhw'
    ,
      name: '1983...(A Merman I Should Turn To Be) / Moon, Turn the Tides...Gently Gently Away'
      author: 'The Jimi Hendrix Experience'
      play: 'https://www.youtube.com/watch?v=rbnE5YAmnCQ'
    ]
  key:
    name: 'The Skeleton Key of Ambady'
    author: 'Adalai Trammels'
    play: 'http://inurashii.xyz/skeleton-key/'
    download: 'http://inurashii.xyz/skeleton-key/skeleton-key.zip'
    songs: [
      name: 'Skeleton Key'
      author: 'Dessa'
      play: 'https://m.youtube.com/watch?v=1_DFCXUGwFE#'
    ]
  submerge:
    name: 'Submerge'
    author: 'Carlos Percival Saldanha'
    play: 'http://www.nigeljayne.ca/scd2downloads/Submerge.html'
    songs: [
      name: 'The Mary Ellen Carter'
      author: 'Stan Rogers'
      play: 'https://www.youtube.com/watch?v=Fhop5VuLDIQ'
    ,
      name: "Jusqu'a la Mort"
      author: 'Barbara Eugenia'
      play: 'https://soundcloud.com/barbara-eugenia/07-jusqua-la-mort'
    ]
  texas:
    name: 'Texas Instruments Theater'
    author: 'Winston Ian Parrish'
    play: 'http://socksmeanie.com/games/twine/Texas.html'
    songs: [
      name: 'Runaway Train'
      author: 'Soul Asylum'
      play: 'https://www.youtube.com/watch?v=NRtvqT_wMeY'
    ,
      name: 'Cyborgs Vs. Robots'
      author: 'Ludo'
      play: 'https://www.youtube.com/watch?v=hPBmPbBUyuU'
    ,
      name: 'I Am Crazy'
      author: 'Mekons'
      play: 'https://youtu.be/U7G7V0PE8T0'
    ,
      name: 'The Ballad of Barry Allen'
      author: "Jim's Big Ego"
      play: 'https://www.youtube.com/watch?v=odU1bHaYNDQ'
    ,
      name: 'The night the pugilist learned how to dance'
      author: 'Sting'
      play: 'https://www.youtube.com/watch?v=Co8S_4eX5ic'
    ,
      name: 'Hide and Seek'
      author: 'Imogen Heap'
      play: 'https://www.youtube.com/watch?v=McDgDlnDX0Y'
    ,
      name: 'Stone on the Water'
      author: 'Badly Drawn Boy'
      play: 'https://www.youtube.com/watch?v=JgrHvz3zZZM'
    ,
      name: '57821'
      author: 'Janelle Monáe'
      play: 'https://www.youtube.com/watch?v=tSFNP8HnjrA'
    ]
  spring:
    name: 'To Spring Open'
    author: 'Two-Bit Chip'
    play: 'http://www.nigeljayne.ca/scd2downloads/ToSpringOpen.html'
    walkthrough: 'http://www.nigeljayne.ca/scd2downloads/ToSpringOpen-Walkthrough.txt'
    songs: [
      name: 'Paper Planes'
      author: 'MIA'
      play: 'https://www.youtube.com/watch?v=ewRjZoRtu0Y'
    ,
      name: 'Tragic Kingdom'
      author: 'No Doubt'
      play: 'https://www.youtube.com/watch?v=avvrRboTerM'
    ]

